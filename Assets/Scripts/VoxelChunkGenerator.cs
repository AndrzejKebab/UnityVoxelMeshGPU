...
public enum RenderMode { DrawProceduralIndirect = 0, RenderPrimitivesIndirect = 1, RenderPrimitivesIndexedIndirect = 2 }
public enum DispatchMode { Dispatch = 0, DispatchIndirect = 1, AsyncCompute = 2 }
...
public class VoxelChunkGenerator : MonoBehaviour
{
    ...
    public int MaxVertices = 5000000;
    public int MaxIndices = 7500000;
    ...
    private ComputeBuffer argsBuffer;    // For Feedback & Voxelizer
    private ComputeBuffer genArgsBuffer; // For GenerateVoxels
    ...
    // standard 3D dispatch for mesh generation
    private int dispatchX, dispatchY, dispatchZ;
    // 2D dispatch for bitpacked generation
    private int genDispatchX, genDispatchY; 
    private int voxelBufferSize;
    private const int maxDispatchCount = 65000; // Stay safely under 65535 limit
    ...
    private void Awake() {
        ...
        for (var i = 0; i <= (int)RenderMode.RenderPrimitivesIndexedIndirect; i++) renderDropdown.options.Add(new TMP_Dropdown.OptionData { text = ((RenderMode)i).ToString() });
        renderDropdown.value = (int)renderingMode;
        
        dispatchDropdown.options.Clear();
        for (var i = 0; i <= (int)DispatchMode.AsyncCompute; i++) dispatchDropdown.options.Add(new TMP_Dropdown.OptionData { text = ((DispatchMode)i).ToString() });
    ...
    private void CalculateDimensions() {
        ...
        // Feedback / Voxelizer bounds
        dispatchX = totalVoxelsX / WORK_GROUP_SIZE;
        dispatchY = totalVoxelsY / WORK_GROUP_SIZE;
        dispatchZ = totalVoxelsZ / WORK_GROUP_SIZE;

        // GenerateVoxels bitpacked bounds (2D Grid mapping to avoid 65535 limitation)
        long totalVoxels = (long)totalVoxelsX * totalVoxelsY * totalVoxelsZ;
        voxelBufferSize = Mathf.CeilToInt((float)totalVoxels / 32f);
        int totalGenGroups = Mathf.CeilToInt((float)voxelBufferSize / 64f);
        
        genDispatchX = Mathf.Min(totalGenGroups, maxDispatchCount);
        genDispatchY = Mathf.CeilToInt((float)totalGenGroups / maxDispatchCount);
    }

    private unsafe void InitializeBuffers() {
        if ((long)voxelBufferSize * sizeof(uint) > 3131047936L) {
            Debug.LogError("Voxel buffer STILL exceeds Unity ComputeBuffer limits. Reduce Grid or Size.");
            return;
        }

        voxelBuffer = new ComputeBuffer(voxelBufferSize, sizeof(uint));
        ...
        verticesBuffer = new ComputeBuffer(MaxVertices, 32); 
        ...
        genArgsBuffer = new ComputeBuffer(1, sizeof(uint) * 3, ComputeBufferType.IndirectArguments, ComputeBufferMode.Immutable);
        genArgsBuffer.SetData(new uint[]{ (uint)genDispatchX, (uint)genDispatchY, 1 });
    }

    private void SetupDrawProceduralIndirect() { indirectBuffer = new ComputeBuffer(1, sizeof(uint) * 5, ComputeBufferType.IndirectArguments, ComputeBufferMode.Immutable); indirectBuffer.SetData(new uint[] { 0, 1, 0, 0, 0 }); }
    private void SetupRenderPrimitivesIndirect() { commandsBuffer = new GraphicsBuffer(GraphicsBuffer.Target.IndirectArguments, 1, GraphicsBuffer.IndirectDrawArgs.size); }
    private void SetupRenderPrimitivesIndexedIndirect() { indexedCommandBuffer = new GraphicsBuffer(GraphicsBuffer.Target.IndirectArguments, 1, GraphicsBuffer.IndirectDrawIndexedArgs.size); }
    ...
    private void SetupAsynCompute() {
        ...
        asyncCommandBuffer.DispatchCompute(GenerateVoxelsComputeShader, generateVoxelsKernelId, genArgsBuffer, 0);
    ...
    private void BindBuffer() {
        ...
        GenerateVoxelsComputeShader.SetInt("uVoxelBufferSize", voxelBufferSize);
        GenerateVoxelsComputeShader.SetInt("uMaxDispatchX", maxDispatchCount);
    ...
    private void SetupRenderParams() {
        renderParams = new RenderParams(drawMaterial) {
            layer = 0, renderingLayerMask = RenderingLayerMask.defaultRenderingLayerMask,
            worldBounds = bounds, camera = cam, shadowCastingMode = ShadowCastingMode.On, receiveShadows = true,
    ...
    private void DispatchShaders() {
        GenerateVoxelsComputeShader.Dispatch(generateVoxelsKernelId, genDispatchX, genDispatchY, 1);
    ...
    private void DispatchIndirectShaders() {
        GenerateVoxelsComputeShader.DispatchIndirect(generateVoxelsKernelId, genArgsBuffer);
    ...
    private void ChangeRenderMode() {
        switch (renderingMode) {
            case RenderMode.DrawProceduralIndirect: DrawProceduralIndirect(); if (drawMaterial.IsKeywordEnabled("RENDER_INDEXED")) drawMaterial.DisableKeyword("RENDER_INDEXED"); break;
            case RenderMode.RenderPrimitivesIndirect: RenderPrimitivesIndirect(); if (drawMaterial.IsKeywordEnabled("RENDER_INDEXED")) drawMaterial.DisableKeyword("RENDER_INDEXED"); break;
            case RenderMode.RenderPrimitivesIndexedIndirect: if (!drawMaterial.IsKeywordEnabled("RENDER_INDEXED")) drawMaterial.EnableKeyword("RENDER_INDEXED"); RenderPrimitivesIndexedIndirect(); break;
            default:
    ...
    private void Reset() { chunkFeedback[0] = new ChunkFeedback(); chunkFeedbackBuffer.SetData(chunkFeedback); }

    private void OnDestroy() {
        voxelBuffer?.Release(); chunkFeedbackBuffer?.Release(); subChunkFeedbackBuffer?.Release();
        verticesBuffer?.Release(); indicesBuffer?.Release(); indirectBuffer?.Release();
        argsBuffer?.Release(); genArgsBuffer?.Release(); commandsBuffer?.Release(); 
        indexedCommandBuffer?.Release(); asyncCommandBuffer?.Release();
    }
}